  ...
  ...)
{
  ...
  if (sum(inherits(object, c("irfsrc", "grow"), TRUE) == c(1, 2)) == 2) {
    if (is.forest.missing(object)) {
      stop("The forest is empty.  Re-run irfsrc (grow call) with forest=TRUE")
    }
  ...
  ## added from generic.predict.irfsrc.R per UBK instruction 11/04/2019
  if (hdim == 0) {
    offset = 0
    chunk = 0
  } else {
  ...
  irfsrcOutput <- list(call = match.call(),
                      family = family,
  ...
  class(irfsrcOutput) <- c("irfsrc", "partial",   family)
  return (irfsrcOutput)
}
partial <- partial.irfsrc
