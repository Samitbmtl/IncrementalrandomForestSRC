stat.split.irfsrc <- function(object, ...)
{
  ...
  if ((sum(inherits(object, c("irfsrc", "grow"), TRUE) == c(1, 2)) != 2) &&
      (sum(inherits(object, c("irfsrc", "predict"), TRUE) == c(1, 2)) != 2))
    stop("This function only works for objects of class `(irfsrc, grow)' or `(irfsrc, pred)',")
