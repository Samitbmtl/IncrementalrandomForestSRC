max.subtree.irfsrc <- function(object,
                              max.order = 2,
                              sub.order = FALSE,
                              conservative = FALSE,
                              ...)
{
  ...
  if (is.null(nativeArray)) {
    stop("irfsrc nativeArray content is NULL.  Please ensure the object is valid.")
  }
  xvar.names <- object$xvar.names
  if (is.null(xvar.names)) {
    stop("irfsrc xvar.names content is NULL.  Please ensure the object is valid.")
  }
  if (is.null(object$xvar)) {
    stop("irfsrc xvar content is NULL.  Please ensure the object is valid.")
  }
  ## verify the max.order option
  max.order <- floor(max.order)
  if (max.order < 0) {
    stop("irfsrc 'max.order' requested for distance order statistic must be an integer greater than zero (0).")
  }
  ...
  subtree.obj <- mclapply(1:numTree, function(b) {
    ...
    recursiveObject <- irfsrcParseTree(
      recursiveObject,
      max(max.order, 1),
      sub.order,
      nativeArray,
...
irfsrcParseTree <- function(recursiveObject,
                           max.order,
                           sub.order,
                           nativeArray,
                           b,
  ...
  if (!terminalFlag) {
    ...
    if (subtreeFlag[splitParameter] == FALSE) {
      ...
      if (sub.order == TRUE) {
        ...
        recursive2Object <- irfsrcParse2Tree(recursive2Object,
                                            nativeArray,
                                            b,
                                            distance=0,
  ...
  if (terminalFlag == FALSE) {
    ...
    distance <- distance + 1
    ## Parse left:
    recursiveObject <- irfsrcParseTree(recursiveObject, max.order, sub.order, nativeArray, b, distance, subtreeFlag)
    ## Parse right:
    recursiveObject <- irfsrcParseTree(recursiveObject, max.order, sub.order, nativeArray, b, distance, subtreeFlag)
  }
    else {
...
irfsrcParse2Tree <- function(recursiveObject,
                            nativeArray,
                            b,
                            distance,
                            subtreeFlag) {
  ...
  if (terminalFlag == FALSE) {
    ## Parse left:
    recursiveObject = irfsrcParse2Tree(recursiveObject, nativeArray, b, distance, subtreeFlag)
    ## Parse right:
    recursiveObject = irfsrcParse2Tree(recursiveObject, nativeArray, b, distance, subtreeFlag)
  }
  return(recursiveObject)
...
max.subtree <- max.subtree.irfsrc
