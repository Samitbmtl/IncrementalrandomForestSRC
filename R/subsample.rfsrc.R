                            ...
                            verbose = TRUE) 
{
  ...
  ##
  ##--------------------------------------------------------------
  if (sum(inherits(obj, c("irfsrc", "grow"), TRUE) == c(1, 2)) != 2) {
...
extract.subsample <- function(obj, alpha = .05, target = 0, m.target = NULL, standardize = TRUE)
{
  ...
  if (sum(c(grepl("irfsrc", class(obj))), grepl("subsample", class(obj))) == 2) {
    subsample <- TRUE
  }
  else if (sum(c(grepl("irfsrc", class(obj))), grepl("bootsample", class(obj))) == 2) {
    subsample <- FALSE
  }
  else {
...
##
###################################################################
print.subsample.irfsrc <- function(x, alpha = .05, standardize = TRUE) {
  vmp <- x$vmp
...
print.subsample <- print.subsample.irfsrc
###################################################################
##
...
extract.bootsample <- function(obj, alpha = .05, target = 0, m.target = NULL, standardize = TRUE)
{
  ...
  if (!sum(c(grepl("irfsrc", class(obj))), grepl("bootsample", class(obj))) == 2) {
...
##
###################################################################
print.bootsample.irfsrc <- function(x, alpha = .05, standardize = TRUE) {
  vmp <- x$vmp
  nullO <- lapply(1:length(vmp), function(j) {
    m.target <- names(vmp)[j]
    p.vmp <- ncol(vmp[[j]])
...
print.bootsample <- print.bootsample.irfsrc
