  ...
  ...)
{
  ...
  if (sum(inherits(object, c("irfsrc", "grow"), TRUE) == c(1, 2)) != 2 &
      sum(inherits(object, c("irfsrc", "plot.variable"), TRUE) == c(1,2)) != 2) {
    stop("this function only works for objects of class `(irfsrc, grow)', '(irfsrc, synthetic)' or '(irfsrc, plot.variable)'")
  }
  ...
  if (partial && (is.null(object$forest) || !object$forest$forest)) {
    stop("forest is empty:  re-run irfsrc (grow call) with forest=TRUE")
  }
  ...
  if (!inherits(object, "plot.variable")) {
      ...
      yhat <- extract.pred(predict(object, m.target = m.target),
        ...
        prtl <- lapply(1:nvar, function(k) {
          ...
          if (!is.factor(x) & n.x > npts) {
            ...
            if (is.factor(x)) {
              x.uniq <- 1:length(unique(x))##partial.irfsrc requires factors as integers
            }
            else {
          ...
          pred.temp <- extract.partial.pred(partial.irfsrc(object$forest,
                                                          m.target = m.target,
    ...
    ## assign the class
    class(plot.variable.obj) <- c("irfsrc", "plot.variable", family)
  }
  ## ---------------------------------------------------------------------------------
...
## using the partial.irfsrc() wrapper, and then extracted without any
...
plot.variable <- plot.variable.irfsrc
