           ...
           ...)
{
  ...
  if (!missing(object)) {
    ...
    ## determine the grow interface - irfsrc or irfsrcFast?
    if (!fast) {
      irfsrc.grow <- "irfsrc"
    }
    else {
      irfsrc.grow <- "irfsrcFast"
    }
  }
  ## --------------------------------------------------------------
  ##   
  ...
  if (missing(object)) {
    ...
    if (oob) {
      ...
      ## the resulting inbag sample depends on whether fast irfsrc is requested
      roughO <- do.call(irfsrc.grow,
