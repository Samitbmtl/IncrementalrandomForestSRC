           ...
           )
{
  ...
  ##
  ## --------------------------------------------------------------
  irfsrc.var.hunting <- function(train.id, var.pt, nstep) {
    ## ------------------filtering step-----------------------
    ...
    irfsrc.filter.obj  <- irfsrc(irfsrc.all.f,
    ...
    if (irfsrc.filter.obj$family == "surv-CR") {
      target.dim <- max(1, min(cause, max(get.event.info(irfsrc.filter.obj)$event.type)), na.rm = TRUE)
    }
    ## extract vimp
    ## for multivariate families we must manually extract the importance and error rate
    imp <- get.varselect.imp(coerce.multivariate(irfsrc.filter.obj, m.target), target.dim)
    names(imp) <- irfsrc.filter.obj$xvar.names
    ...
    ## selection using minimal depth
        ...
        max.obj <- max.subtree(irfsrc.filter.obj, conservative = (conservative == "high"))
        if (is.null(max.obj$order)) {
          ...
          sig.vars.old <- names(sort(imp, decreasing = TRUE))[1]
            ...
            names(m.depth) <- names(VarStrength) <- irfsrc.filter.obj$xvar.names
    ...
    ## set nstep
    nstep <- max(round(length(irfsrc.filter.obj$xvar.names)/nstep), 1)
    imp.old <- 0
    ...
    for (b in 1:nstep) {
      ...
      imp <- coerce.multivariate(vimp(irfsrc.filter.obj, sig.vars, m.target = m.target,
    ...
    var.pt <- var.columns[match(sig.vars, xvar.names)]
    drop.var.pt <- setdiff(var.columns, var.pt)
    irfsrc.obj  <- irfsrc(irfsrc.all.f,
                        data=(if (LENGTH(var.pt, drop.var.pt)) data[train.id, -drop.var.pt]
                                else data[train.id, ]),
                        ntree = ntree,
                        splitrule = splitrule,
                        ...
                        na.action = na.action,
                        perf.type = perf.type)
    return(list(irfsrc.obj=irfsrc.obj, sig.vars=irfsrc.obj$xvar.names, forest.depth=forest.depth, m.depth=m.depth))
  }
  ## --------------------------------------------------------------
  ##   
  ...
  if (missing(object)) {
    ## parse the formula
    formulaDetail <- finalizeFormula(parseFormula(irfsrc.all.f, data), data)
  ...
  if (method == "md") {
    ...
    if (prefit.flag && is.null(xvar.wt) && missing(object)) {
      ...
      irfsrc.prefit.obj  <- irfsrc(irfsrc.all.f,
                                 data = data,
                                 ntree = prefit$ntree,
                                 ...
                                 block.size = block.size, perf.type = perf.type)
      ## set the target dimension for CR families
      if (irfsrc.prefit.obj$family == "surv-CR") {
        target.dim <- max(1, min(cause, max(get.event.info(irfsrc.prefit.obj)$event.type)), na.rm = TRUE)
      }
      ## for multivariate families we must manually extract the importance and error rate
      wts <- pmax(get.varselect.imp(coerce.multivariate(irfsrc.prefit.obj, m.target), target.dim), 0)
      if (any(wts > 0)) {
        xvar.wt <- get.weight(wts, P)
      }
      rm(irfsrc.prefit.obj)
    }
    ## ------------------------------------------------
    ...
    ## otherwise run forests...then extract minimal depth    
        ...
        irfsrc.obj <- irfsrc(irfsrc.all.f,
                           ...
                           block.size = block.size, perf.type = perf.type)
        ## set the target dimension for CR families
        if (irfsrc.obj$family == "surv-CR") {
          target.dim <- max(1, min(cause, max(get.event.info(irfsrc.obj)$event.type)), na.rm = TRUE)
        }
        if (verbose) cat("minimal depth variable selection ...\n")
        md.obj <- max.subtree(irfsrc.obj, conservative = (conservative == "high"))
        ## for multivariate families we must manually extract the importance and error rate
        irfsrc.obj <- coerce.multivariate(irfsrc.obj, m.target)
        m.target <- irfsrc.obj$outcome.target
        pe <- get.varselect.err(irfsrc.obj)
        imp <- get.varselect.imp(irfsrc.obj, target.dim)
        imp.all <- get.varselect.imp.all(irfsrc.obj)
        mtry <- irfsrc.obj$mtry
        nodesize <- irfsrc.obj$nodesize
        n <- nrow(irfsrc.obj$xvar)
        family <- irfsrc.obj$family
        rm(irfsrc.obj)#don't need the grow object
      }
    ...
    if (refit == TRUE) {
      ...
      drop.var.pt <- setdiff(var.columns, var.pt)
      irfsrc.refit.obj  <- irfsrc(irfsrc.all.f,
                                ...
                                perf.type = perf.type)
      ## for multivariate families we must manually extract the importance and error rate
      irfsrc.refit.obj <- coerce.multivariate(irfsrc.refit.obj, m.target)
    }
      else {
        irfsrc.refit.obj <- NULL
      }
    ...
    return(invisible((list(err.rate=pe,
                           ...
                           topvars=topvars,
                           varselect=varselect,
                           irfsrc.refit.obj=irfsrc.refit.obj,
                           md.obj=md.obj
                           ))))
  }  
  ## --------------------------------------------------------------
  ...
  for (m in 1:nrep) {
    ...
    if (is.null(xvar.wt)) {
      ...
      ## for multivariate families we must manually extract the importance and error rate
      irfsrc.prefit.obj <- coerce.multivariate(irfsrc.prefit.obj, m.target)
      wts <- pmax(get.varselect.imp(irfsrc.prefit.obj, target.dim), 0)
      if (any(wts > 0)) {
    ...
    ## irfsrc gene hunting call
    object <- irfsrc.var.hunting(train.id, var.pt, nstep)
    irfsrc.obj <- object$irfsrc.obj
    m.target <- get.univariate.target(irfsrc.obj, m.target)
    ...
    ## irfsrc prediction
    ## for multivariate families we must manually extract the importance and error rate
    pred.out <- coerce.multivariate(predict(irfsrc.obj, data[test.id, ]), m.target)
