plot.survival.irfsrc <- function (x,
                                 plots.one.page = TRUE,
                                 show.plots = TRUE,
                                 subset, collapse = FALSE,
                                 cens.model = c("km", "irfsrc"),
                                 ...)
{
  ...
  if (sum(inherits(x, c("irfsrc", "predict"), TRUE) == c(1, 2)) == 2) {
    pred.flag <- TRUE
  }
  else {
    pred.flag <- FALSE
  ...
  cens.model <- match.arg(cens.model, c("km", "irfsrc"))
  ...
  if (!pred.flag && !subset.provided) {
    ...
    if (length(censTime) > 0) {
      ...
      ## irfsrc estimator for the censoring distribution
      else {
        cens.dist <- irfsrc(Surv(tm, cens) ~ .,
...
plot.survival <- plot.survival.irfsrc
## trapezoidal rule
trapz <- function (x, y) {
  idx = 2:length(x)
