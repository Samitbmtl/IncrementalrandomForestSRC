tune.nodesize.irfsrc <- function(formula, data, 
                            ...
                            nsplit = 1, trace = FALSE, ...) 
{
  ...
  err <- sapply(nodesizeTry, function(nsz) {
    ...
    err.nsz <- tryCatch({mean(get.mv.error(irfsrc.fast(formula, data, nodesize = nsz,
  ...
  return(list(nsize.opt = nsize.opt,
              err = data.frame(nodesize = nodesizeTry, err = err)))
}
tune.nodesize <- tune.nodesize.irfsrc
