quantileReg.irfsrc <- function(formula, data, object, newdata, method = "forest",
                              ...
                              oob = TRUE, fast = FALSE, maxn = 1e3, ...)
{
  ...
  if (missing(object)) {
    ...
    object <- do.call(irfsrc.grow, c(list(formula = formula, data = data, splitrule = splitrule), dots))
  }
...
quantileReg <- quantileReg.irfsrc
