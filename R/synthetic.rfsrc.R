           ...
           ...)
{
  ...
  if (!missing(object)) {
    ...
    if (sum(inherits(object, c("irfsrc", "synthetic"), TRUE) == c(1, 2)) != 2) {
      stop("this function only works for objects of class `(irfsrc, synthetic)'")
    }
    ...
    preObj <- irfsrc(f.org, data, ntree = 1, nodesize = nrow(data), splitrule = "random")
    fmly <- preObj$family
    ...
    ## determine the grow interface - irfsrc or irfsrc.fast?
    if (!fast) {
      irfsrc.grow <- "irfsrc"
    }
    else {
      irfsrc.grow <- "irfsrc.fast"
    }
  }
  ...
  if (missing(object)) {
    ...
    if (oob) {
      ...
      ## inbag sample depends on whether irfsrc.fast is requested or default sampling used by irfsrc
      ## note that this is now "swor" in both cases
      roughO <- do.call("irfsrc",
  ...
  if (oob) {
    class(retObj) <- c("irfsrc", "synthetic", "oob")
  }
  else {
    class(retObj) <- c("irfsrc", "synthetic", "inb")
  }
  retObj
}
## --------------------------------------------------------------
