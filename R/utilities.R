...
get.seed <- function (seed) {
  ...
  get.OOB <- function (OOB) {
    ## Convert OOB into native code parameter.
    if (!is.logical(OOB)) {
      if (OOB >= 1) {
        OOB <- round(OOB)
      }
  ...
  #Sami
  get.obsUniqueInTree <- function (obsUniqueInTree) {
    ## Convert obsUniqueInTree into native code parameter.
    if (!is.logical(obsUniqueInTree)) {
      if (obsUniqueInTree >= 1) {
        obsUniqueInTree <- round(obsUniqueInTree)
      }
      else {
        obsUniqueInTree <- 0
      }
      ...
      obsUniqueInTree <- 1 * obsUniqueInTree
    }
    return (obsUniqueInTree)
  }  
