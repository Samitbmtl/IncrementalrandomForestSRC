...
{
  ##Sami
  ObjMembership <- object$membership
  Objinbag <- object$inbag
  #Objyvar <- as.matrix(object$yvar)
  Objyvar <- object$yvar
  Objxvar <- object$xvar
  if (object$family == "IndSurvB" || object$family == "HET" || object$family == "indQuadC" || object$family == "indQuad2C" || object$family == "indLSC" || object$family == "indTSTC" || object$family == "indTST2C" || object$family == "indNSRC" || object$family == "indMOB0C" || object$family == "CMB" || object$family == "indMOBQC" || object$family == "indMaxLC" || object$family == "indMaxQC" || object$family == "indMaxMaxQC" || object$family == "indCat" || object$family == "IndSurvCat")
  {
    membership = TRUE
  }
  ...
  if (sum(inherits(object, c("irfsrc", "grow"), TRUE) == c(1, 2)) != 2    &
      sum(inherits(object, c("irfsrc", "forest"), TRUE) == c(1, 2)) != 2)
    stop("this function only works for objects of class `(irfsrc, grow)' or '(irfsrc, forest)'")
  ## grow forests must have true forest information
  if (sum(inherits(object, c("irfsrc", "grow"), TRUE) == c(1, 2)) == 2) {
    if (is.forest.missing(object)) {
      stop("Forest information for prediction is missing.  Re-run irfsrc (grow call) with forest=TRUE")
    }
  }
  ...
  if (is.null(object$version)) {
    ...
    stop()
      ...
      installed.version <- as.integer(unlist(strsplit("2.9.2", "[.]")))
  ...
  nativeOutput <- tryCatch({.Call("rfsrcPredict",
                                  ...
                                  if (family == "indLSC") as.integer(1) else as.integer(r.dim),
                                  ...
                                  as.integer(samp),
                                  as.double(case.wt),
                                  list(if(is.null(event.info$time.interest) || family == "indLSC") as.integer(0) else as.integer(length(event.info$time.interest)),
                                       if(is.null(event.info$time.interest) || family == "indLSC") NULL else as.double(event.info$time.interest)),
                                  ...
                                  ## irfsrc.R after the nativeArray
                                  ...
                                  # list(if (is.null(m.target.idx) || family == "indLSC") as.integer(0) else as.integer(length(m.target.idx)),
                                  #      if (is.null(m.target.idx) || family == "indLSC") NULL else as.integer(m.target.idx)),
                                  ...
                                  if (family == "indLSC") as.integer(1) else as.integer(r.dim.newdata),
                                  #as.integer(r.dim.newdata),
                                  as.double(if (outcome != "test") (if (family == "indLSC") yvar.newdata[1] else yvar.newdata) else NULL),
  ...
  if (grepl("surv", family) || grepl("IndSurvB", family) || grepl("HET", family) || grepl("indQuadC", family) || grepl("indQuad2C", family) || grepl("indLSC", family) || grepl("indTSTC", family) || grepl("indTST2C", family) || grepl("indNSRC", family)  || grepl("indMOB0C", family) || grepl("CMB", family) || grepl("indMOBQC", family) || grepl("indMaxLC", family) || grepl("indMaxQC", family) || grepl("indMaxMaxQC", family) || grepl("indCat", family) || grepl("IndSurvCat", family)) {
	...
	##Sami
	if (grepl("IndSurvB", irfsrcOutput$family))
	{
		#predicted <- Reduce(c,parallel::mclapply(1:nrow(irfsrcOutput$membership), fun_pred,Objyvar,ObjMembership,irfsrcOutput$membership,Objinbag))

# 	  no_cores <-parallel::detectCores()
# 	  cl_compile<- parallel::makeCluster(no_cores)
# 	  doParallel::registerDoParallel(cl_compile)
# 	  predicted <- foreach::foreach(i=1:nrow(irfsrcOutput$membership), .combine = rbind) %dopar% 
#     {
#       fun_pred(i,Objyvar,ObjMembership,irfsrcOutput$membership,Objinbag)
#     }
# 	  parallel::stopCluster(cl_compile)
	  
	  # for(i in 1:nrow(irfsrcOutput$membership))
	  # {
	  #   predicted <- fun_pred(i,Objyvar,ObjMembership,irfsrcOutput$membership,Objinbag, !(OOB))
	  # }
	          
	  	  no_cores <-parallel::detectCores()
	  	  cl_compile<- parallel::makeCluster(no_cores)
	  	  predicted <- parallel::parSapply(cl_compile,1:nrow(irfsrcOutput$membership), fun_pred,Objyvar,ObjMembership,irfsrcOutput$membership,Objinbag, !(OOB))
	  	  parallel::stopCluster(cl_compile)
	}
	if (grepl("IndSurvCat", irfsrcOutput$family))
	{
	  	  # no_cores <-parallel::detectCores()
	  	  # cl_compile<- parallel::makeCluster(no_cores)
	  	  # predicted <- parallel::parSapply(cl_compile,1:nrow(irfsrcOutput$membership), fun_pred,Objyvar,ObjMembership,irfsrcOutput$membership,Objinbag, !(OOB))
	  ...
	  predicted <- fun_pred_SurvCat(iextractBBOP(Objyvar,ObjMembership,irfsrcOutput$membership,Objinbag,!(OOB),obsUniqueInTree))
	}
  ...
  class(irfsrcOutput) <- c("irfsrc", "predict",   family)
  return(irfsrcOutput)
}
