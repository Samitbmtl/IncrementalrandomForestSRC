irfsrc <- function(formula, data, ntree = 1000,
                  mtry = NULL, ytry = NULL,
                   ...
                   ...)
{
    ...
    ## Set the cause weights for the split statistic calculation.
	  ##Sami
    if (family == "IndSurvB" || family == "HET" || family == "indQuadC" || family == "indQuad2C" || family == "indLSC" || family == "indTSTC" || family == "indTST2C" || family == "indNSRC" || family == "indMOB0C" || family == "CMB" || family == "indMOBQC" || family == "indMaxLC" || family == "indMaxQC" || family == "indMaxMaxQC" || family == "indCat" || family == "IndSurvCat" )
    {
      membership = TRUE
      #for all non survival method, adjust for centering parameters
      if(family != "IndSurvB")
      {
        if(incremental.CenterResponse == TRUE)
        {
          xnames <- ""
          ...
          eval(parse(text = paste0("rfy <- randomForestSRC::rfsrc(",formulaDetail$yvar.names[1],"~ " , xnames , ", data = data)")))
          #rfy <- randomForestSRC::rfsrc(formulaDetail$yvar.names[1] ~ xvar.names, data = data)
          prr <- predict(rfy)
          yvar[,1] <- yvar[,1] - prr$predicted.oob
        }
        
        if(incremental.CenterTreatment == TRUE)
        {
          xnames <- ""
          for (i in 1 : length(xvar.names))
          {
          ...
          eval(parse(text = paste0("rfg <- randomForestSRC::rfsrc(",formulaDetail$yvar.names[2],"~ " , xnames , ", data = data)")))
          #rfy <- randomForestSRC::rfsrc(formulaDetail$yvar.names[1] ~ xvar.names, data = data)
          prr <- predict(rfg)
          yvar[,2] <- yvar[,2] - prr$predicted.oob
        }
      }
    }
  	else
    ...
    if (family == "surv" || family == "surv-CR" || family == "IndSurvB" || family == "IndSurvCat") {
    ...
    if (impute.only) {
        ...
        ## impute.irfsrc( generic.impute.irfsrc( irfsrc() ) ).  Now, the user specified option
        ## is respected.  Note that calls from impute.irfsrc()
    ...
    ##Sami
	  contol.incremental(family,nodesize,incremental.tailType,incremental.minObsControl,incremental.minObsTreatment,incremental.TreatmentLowBound,incremental.TreatmentUpperBound)
    ...
    nativeOutput <- tryCatch({.Call("rfsrcGrow",
                                    ...
                                    if (family == "indLSC") as.integer(1) else as.integer(formulaDetail$ytry),
                                    ...
                                    as.integer(ntree),
                                    as.integer(n),
                                    if (family == "indLSC") as.integer(1) else as.integer(length(yvar.types)),
                                    if (family == "indLSC") as.character(yvar.types[1]) else as.character(yvar.types),
                                    if (family == "indLSC") as.integer(yvar.nlevels[1]) else as.integer(yvar.nlevels),
                                    if (family == "indLSC") as.double(as.vector(yvar[,1])) else as.double(as.vector(yvar)),
                                    ...
                                    as.double(case.wt),
                                    as.double(split.wt),
                                    if (family == "indLSC") as.double(yvar.wt[1]) else as.double(yvar.wt),
                                    #as.double(yvar.wt),
                                    as.double(xvar.wt),
                                    as.double(xvar),
                                    if (family == "indLSC") as.integer(0) else as.integer(length(event.info$time.interest)),
                                    #as.integer(length(event.info$time.interest)),
                                    if (family == "indLSC") as.double(NULL) else as.double(event.info$time.interest),
                                    ...
                                    as.integer(get.rf.cores()),
                  									as.integer(incremental.tailType),
                  									as.integer(incremental.minObsControl),
                  									as.integer(incremental.minObsTreatment),
                  									as.double(incremental.TreatmentLowBound),
                  									as.double(incremental.TreatmentUpperBound)
                  									)}, error = function(e) {
                                        print(e)
                                        NULL})
    ...
    if (is.null(nativeOutput)) {
        if (impute.only) {
            ...
            ## currently all other modes will error out with the irfsrc error message
    ...
    if (n.miss > 0) {
        ...
        if (nimpute > 1) {
			...
			##Sami
			else if (grepl("IndSurvB", family) || grepl("IndSurvCat", family)) {
                yvar[imputed.indv, 1] <- imputed.data[, 1]
                yvar[imputed.indv, 2] <- imputed.data[, 2]
                xvar[imputed.indv, ] <- imputed.data[, -c(1:2), drop = FALSE]
            }
			else if (grepl("HET", family) || grepl("indQuadC", family) || grepl("indQuad2C", family) || grepl("indTSTC", family) || grepl("indTST2C", family) || grepl("indNSRC", family) || grepl("indMOB0C", family) || grepl("CMB", family) || grepl("indMOBQC", family) || grepl("indMaxLC", family) || grepl("indMaxQC", family) || grepl("indMaxMaxQC", family) || grepl("indCat", family)) {
                yvar[imputed.indv, 1] <- imputed.data[, 1]
                yvar[imputed.indv, 2] <- imputed.data[, 2]
                xvar[imputed.indv, ] <- imputed.data[, -c(1:2), drop = FALSE]
            }
    ...
    if (forest) {
        ...
        if (terminal.qualts | terminal.quants) {
            ...
            if (terminal.quants) {
				...
				##Sami
				else if (grepl("IndSurvB", family) || grepl("HET", family) || grepl("indQuadC", family) || grepl("indQuad2C", family) || grepl("indLSC", family) || grepl("indTSTC", family) || grepl("indTST2C", family) || grepl("indNSRC", family) || grepl("indMOB0C", family) || grepl("CMB", family) || grepl("indMOBQC", family) || grepl("indMaxLC", family) || grepl("indMaxQC", family) || grepl("indMaxMaxQC", family) || grepl("indCat", family) || grepl("IndSurvCat", family)) {
                    offset <- 0
                    valid.2D.surv.indices <- unlist(lapply(1:ntree, function(b) {
                        if (b > 1) {
                            offset <<- offset + (treeTheoreticalMaximum * length(event.info$event.type) * length(event.info$time.interest))
      ...
      class(forest.out) <- c("irfsrc", "forest", family)
      if (big.data) {
  ...
  if (proximity != FALSE) {
	...
	incremental <- list(tailType = incremental.tailType,
						minObsControl = incremental.minObsControl
						,minObsTreatment = incremental.minObsTreatment
						,TreatmentLowBound = incremental.TreatmentLowBound
						,TreatmentUpperBound = incremental.TreatmentUpperBound)
    ...
    classOutput <- NULL
    regrOutput <- NULL
	if (!(grepl("IndSurvB", family) || grepl("HET", family) || grepl("indQuadC", family) || grepl("indQuad2C", family) || grepl("indLSC", family) || grepl("indTSTC", family) || grepl("indTST2C", family) || grepl("indNSRC", family) || grepl("indMOB0C", family) || grepl("CMB", family) || grepl("indMOBQC", family) || grepl("indMaxLC", family) || grepl("indMaxQC", family) || grepl("indMaxMaxQC", family) || grepl("indCat", family) || grepl("IndSurvCat", family)))
	{
		irfsrcOutput$incremental <- NULL
	}
    ## EFFICIENCY EFFICIENCY EFFICIENCY
    ## for efficiency - the following does not need to be executed in impute.only mode
    if (!impute.only) {
        ## family specific additions to the grow object
        if (grepl("surv", family) || grepl("IndSurvB", family) || grepl("HET", family) || grepl("indQuadC", family) || grepl("indQuad2C", family) || grepl("indLSC", family) || grepl("indTSTC", family) || grepl("indTST2C", family) || grepl("indNSRC", family) || grepl("indMOB0C", family) || grepl("CMB", family) || grepl("indMOBQC", family) || grepl("indMaxLC", family) || grepl("indMaxQC", family) || grepl("indMaxMaxQC", family) || grepl("indCat", family) || grepl("IndSurvCat", family)) {
            if ((length(event.info$event.type) > 1) &&
            ...
            if (!is.null(nativeOutput$holdoutSurv)) {
                holdout.vimp <- vector("list", length(irfsrcOutput$holdout.blk))
                names(holdout.vimp) <- xvar.names
                ## These are lengths of of each x-var dimension.    
                holdout.offset <- irfsrcOutput$holdout.blk * length(event.info$event.type) 
                holdout.offset.sum <- c(0, cumsum(holdout.offset))
                for (i in 1:length(holdout.vimp)) {
                    if (irfsrcOutput$holdout.blk[i] > 0) {
                        if (length(event.info$event.type) > 1) {
                            holdout.vimp[[i]] <- array(nativeOutput$holdoutSurv[(holdout.offset.sum[i] + 1) : holdout.offset.sum[i+1]], c(length(event.info$event.type), irfsrcOutput$holdout.blk[i]))
                        }
                        else {
            ...
            if(univariate.nomenclature) {
                irfsrcOutput <- c(irfsrcOutput, survOutput)
            }
            else {
                irfsrcOutput <- c(irfsrcOutput, survOutput = list(survOutput))
            }
        }
        else {
            ...
            if (resp.clas.count > 0) {
                ...
                if (!is.null(nativeOutput$holdoutClas)) {
                    ## These are lengths of of each x-var dimension.    
                    holdout.offset.x <- irfsrcOutput$holdout.blk * (sum(1 + levels.count))
                ...
                for (i in 1:resp.clas.count) {
                    ...
                    if (!is.null(nativeOutput$holdoutClas)) {    
                        ...
                        for (k in 1:length(holdout.vimp)) {
                            ...
                            if (irfsrcOutput$holdout.blk[k] > 0) {
                                ...
                                for (m in 1:irfsrcOutput$holdout.blk[k]) {
                                    if (m > 1) {
                                ...
                                holdout.vimp[[k]] = array(nativeOutput$holdoutClas[index.m], c(levels.count[i] + 1, irfsrcOutput$holdout.blk[k]))
                            }
                            else {
                                holdout.vimp[[k]] = NA
                            }
                ...
                if(univariate.nomenclature) {
                    if ((resp.clas.count == 1) & (resp.regr.count == 0)) {
                        ...
                        irfsrcOutput <- c(irfsrcOutput, unlist(classOutput, recursive=FALSE))
                    }
                    else {
                        irfsrcOutput <- c(irfsrcOutput, classOutput = list(classOutput))
                    }
                }
                else {
                    irfsrcOutput <- c(irfsrcOutput, classOutput = list(classOutput))
            ...
            if (resp.regr.count > 0) {
                ...
                if (!is.null(nativeOutput$holdoutRegr)) {
                    ## These are lengths of of each x-var dimension.    
                    holdout.offset.x <- irfsrcOutput$holdout.blk * resp.regr.count
                    ## This is the cumulative offset for each x-var dimension.
                    holdout.offset.sum.x <- c(0, cumsum(holdout.offset.x))
                    ## Relative response offset.
                    holdout.offset.r <- 0
                ...
                if(univariate.nomenclature) {
                    if ((resp.clas.count == 0) & (resp.regr.count == 1)) {
                        ...
                        irfsrcOutput <- c(irfsrcOutput, unlist(regrOutput, recursive=FALSE))
                    }
                    else {
                        irfsrcOutput <- c(irfsrcOutput, regrOutput = list(regrOutput))
                    }
                }
                else {
                    irfsrcOutput <- c(irfsrcOutput, regrOutput = list(regrOutput))
    ...
    class(irfsrcOutput) <- c("irfsrc", "grow", family)
    if (big.data) {
        class(irfsrcOutput) <- c(class(irfsrcOutput), "bigdata")
    }
    return(irfsrcOutput)
}
