...
family.pretty <- function(x) {
  ...
  switch(fmly,
         ...
         "mix+"     = "mRF-RC",
         "rfq"      = "RFQ",
         "IndSurvB"      = "Individualized Survival Binarry",
         "HET" 	 = "Individualized Linear Continue",
         "indQuadC"      = "Individualized Quadratic Continue",
         "indQuad2C"     = "Individualized Quadratic 2 Continue",
         "indTSTC"       = "Individualized TST (Test)",
         "indNSRC"       = "Individualized NSR(new split rule)",
         "indTST2C"      = "Individualized TST (Test 2)",
         "indLSC"        = "Individualized RF-R (mse)",
		 "indMOB0C"      = "Individualized mse equivalent",
         "CMB"      = "Individualized MOB Linear",
         "indMOBQC"      = "Individualized MOB Quadratic",
         "indMaxLC"      = "Individualized Max Linear",
         "indMaxQC"      = "Individualized Max Quadratic",
         "indMaxMaxQC"   = "Individualized Max Max Quadratic",
		 "indCat"        = "Individualized Categorical",
         "IndSurvCat"    = "Individualized Survival Categorical"
         )
}
...
get.cindex <- function (time, censoring, predicted, do.trace = FALSE) {
  ...
  if (is.null(nativeOutput)) {
    stop("An error has occurred in irfsrcCIndex.  Please turn trace on for further analysis.")
  }
...
get.coerced.survival.fmly <- function(fmly, event.type, splitrule = NULL) {
  ...
  else if (grepl("IndSurvB", fmly)) {
	coerced.fmly <- "IndSurvB"
  }
  else if (grepl("IndSurvCat", fmly)) {
	coerced.fmly <- "IndSurvCat"
  }
  else {
...
get.event.info <- function(obj, subset = NULL) {
  ## survival case
  if (grepl("surv", obj$family) || grepl("IndSurvB", obj$family) || grepl("IndSurvCat", obj$family)) {
    if (!is.null(obj$yvar)) {
      if (is.null(subset)) {
	  ...
	  group <- NULL
	  if (grepl("IndSurvB", obj$family) || grepl("IndSurvCat", obj$family))
	  {
		r.dim <- 3
		group <- obj$yvar[subset, 3]
	  }
	  ...
	  if (grepl("IndSurvB", obj$family))
	  {
		  if (max(cens) != 1 && min(cens) != 0) {
			stop("for IndSurvB censoring should be coded 0/1")
		  }
		  if (max(group) != 1 && min(group) != 0) {
			stop("for IndSurvB group should be coded 0/1")
		  }
	  }
	  if (grepl("IndSurvCat", obj$family))
	  {
		  if (max(cens) != 1 && min(cens) != 0) {
			stop("for IndSurvCat censoring should be coded 0/1")
		  }
		  #### Sami todo : group categorical
		  ##if (max(group) != 1 && min(group) != 0) {
			##stop("for IndSurvCat group should be coded 0/1")
		  ##}
	  }
    ...
    ##everything else
        ...
        r.dim <- 0
        event <- event.type <- cens <- cens <- time <- group <- NULL
      }
    ## Set grid of time points.
	##Sami
	if (is.null(obj$time.interest))
		time.interest <- sort(unique(time))
	else
		time.interest <- obj$time.interest
	
  }
  else if (grepl("HET", obj$family) || grepl("indQuadC", obj$family) || grepl("indQuad2C", obj$family) || grepl("indLSC", obj$family)  || grepl("indTSTC", obj$family)  || grepl("indTST2C", obj$family) || grepl("indNSRC", obj$family) || grepl("indMOB0C", obj$family) || grepl("CMB", obj$family) || grepl("indMOBQC", obj$family) || grepl("indMaxLC", obj$family) || grepl("indMaxQC", obj$family) || grepl("indMaxMaxQC", obj$family) || grepl("indCat", obj$family)) {
    if (!is.null(obj$yvar)) {
      if (is.null(subset)) {
        subset <- (1:nrow(cbind(obj$yvar)))
      }
      ...
      time <- obj$yvar[subset, 1]
      cens <- NULL
	    group <- obj$yvar[subset, 2]
	    #Sami : a la demande de Denis
		#if (min(group) < 0) {
	    #  stop("Treatment variable should be positive (>0)")
	    #}
	...
	if (is.null(obj$time.interest))
      ...
      event <- event.type <- cens <- time.interest <- cens <- time <- group <- NULL
    }
  return(list(event = event, event.type = event.type, cens = cens,
              time.interest = time.interest, time = time, r.dim = r.dim, group = group))
}
...
##Sami
contol.incremental <- function (family,nodeMinSize,incremental.tailType,incremental.minObsControl,incremental.minObsTreatment,incremental.TreatmentLowBound,incremental.TreatmentUpperBound) {
	if (grepl("IndSurvB", family)) {
		if (incremental.tailType != 0 && incremental.tailType != 1)
		{
			stop("tail type should be 0 (none) or 1 (Exponential)")
		}
		if (incremental.minObsControl < 1 || incremental.minObsControl > nodeMinSize)
		{
			stop("incremental.minObsControl should be between 1 and nodesize")
		}
		if (incremental.minObsTreatment < 1 || incremental.minObsTreatment > nodeMinSize)
		{
			stop("incremental.minObsTreatment should be between 1 and nodesize")
		}
		if (incremental.TreatmentLowBound < 0 || incremental.TreatmentLowBound > incremental.TreatmentUpperBound)
		{
			stop("incremental.TreatmentLowBound should be between 0 and incremental.TreatmentUpperBound")
		}
		if (incremental.TreatmentUpperBound > 1 || incremental.TreatmentUpperBound < incremental.TreatmentLowBound)
		{
			stop("incremental.TreatmentUpperBound should be between TreatmentLowBound and 1")
		}
	}
	
	#Sami todo : control mettre
	if (grepl("IndSurvCat", family)) {
		if (incremental.tailType != 0 && incremental.tailType != 1)
		{
			stop("tail type should be 0 (none) or 1 (Exponential)")
		}
...
get.grow.event.info <- function(yvar, fmly, need.deaths = TRUE, ntime) {
  ...
  ## handle new Individualized
	...
	if (need.deaths && (all(na.omit(group) == 0))) {
      stop("2 groups should be there!")
    }
	
	if (max(cens) != 1 && min(cens) != 0) {
		stop("for incremental families censoring should be coded 0/1")
	}
	if (max(group) != 1 && min(group) != 0) {
  ...
  if (length(sort(unique(cens))) == 2 && (sort(unique(cens))[1] != 0 || sort(unique(cens))[2] != 1))
  ...
  else if (grepl("indQuadC", fmly) || grepl("indQuad2C", fmly) || grepl("indMaxQC", fmly) || grepl("indMaxMaxQC", fmly) || grepl("indMOBQC", fmly) ) {
    r.dim <- 2
    time <- yvar[, 1]
    cens <- NULL
    group <- yvar[, 2]

	if (min(group) < 0 || max(group) > 1) {
      stop("Treatment variable should be in [0,1]")
    }
    ...
    event.type <- NULL
    ## Set grid of time points.
    nonMissingOutcome <- NULL
    nonMissingDeathFlag <- NULL
    time.interest <- sort(unique(time))
    ## trim the time points if the user has requested it
    ## we also allow the user to pass requested time points
    if (!missing(ntime)) {
  ...
  else if (grepl("HET", fmly) || grepl("indLSC", fmly) || grepl("indTSTC", fmly) || grepl("indTST2C", fmly) || grepl("indNSRC", fmly) || grepl("indMOB0C", fmly) || grepl("CMB", fmly) || grepl("indMaxLC", fmly) || grepl("indCat", fmly)) {
    r.dim <- 2
    time <- yvar[, 1]
    cens <- NULL
    group <- yvar[, 2]
  ...
  return(list(event = event, event.type = event.type, cens = cens,
              time.interest = time.interest,
              time = time, r.dim = r.dim, group = group))
}
...
get.grow.nodesize <- function(fmly, nodesize) {
  ...
  ## Default node size for unsupervised splitting
			...
			else if (fmly == "IndSurvB") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			else if (fmly == "IndSurvCat") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			...
			else if (fmly == "indQuadC") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			else if (fmly == "indQuad2C") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			...
			else if (fmly == "indNSRC") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			else if (fmly == "indMOB0C") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			...
			else if (fmly == "indMOBQC") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			else if (fmly == "indMaxLC") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			else if (fmly == "indMaxQC") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			else if (fmly == "indMaxMaxQC") {
              if (is.null(nodesize)) {
                nodesize <- 50
              }
            }
			else if (fmly == "indTSTC" || fmly == "indTST2C") {
              if (is.null(nodesize)) {
                nodesize <- 5
              }
			}
...
get.grow.splitinfo <- function (formula.detail, splitrule, hdim, nsplit, event.type) {
  ...
  splitrule.names <- c("logrank",              ##  1
                       ...
                       "sg.surv",              ## 19
                       "IndSurvBSpecific",	   ## 20
                       "indLinearCSpecific",   ## 21 
                       "indQuadCSpecific",     ## 22 
                       "indQuad2CSpecific",    ## 23 
                       "indTSTCSpecific",      ## 24 
                       "indNSRCSpecific",      ## 25 
                       "indTST2CSpecific",     ## 26 --
                       "indMOB0CSpecific",     ## 27 
                       ...
                       "indMaxLCSpecific",     ## 30 
                       "indMaxQCSpecific",     ## 31 
                       "indMaxMaxQCSpecific",  ## 32
                       "indCatSpecific",       ## 33
                       "IndSurvCatSpecific"    ## 34
					   )     
  ...
  if (!splitpass) {
	...
	if (grepl("IndSurvB", fmly)) {
		splitrule.idx <- which(splitrule.names == "IndSurvBSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("IndSurvCat", fmly)) {
		splitrule.idx <- which(splitrule.names == "IndSurvCatSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("HET", fmly)) {
		splitrule.idx <- which(splitrule.names == "indLinearCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indQuadC", fmly)) {
		splitrule.idx <- which(splitrule.names == "indQuadCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indQuad2C", fmly)) {
		splitrule.idx <- which(splitrule.names == "indQuad2CSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indLSC", fmly)) {
		splitrule.idx <- which(splitrule.names == "mse")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indTSTC", fmly)) {
		splitrule.idx <- which(splitrule.names == "indTSTCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indTST2C", fmly)) {
		splitrule.idx <- which(splitrule.names == "indTST2CSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indNSRC", fmly)) {
		splitrule.idx <- which(splitrule.names == "indNSRCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indMOB0C", fmly)) {
		splitrule.idx <- which(splitrule.names == "indMOB0CSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("CMB", fmly)) {
		splitrule.idx <- which(splitrule.names == "indMOBLCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indMOBQC", fmly)) {
		splitrule.idx <- which(splitrule.names == "indMOBQCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indMaxLC", fmly)) {
		splitrule.idx <- which(splitrule.names == "indMaxLCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indMaxQC", fmly)) {
		splitrule.idx <- which(splitrule.names == "indMaxQCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
	if (grepl("indMaxMaxQC", fmly)) {
		splitrule.idx <- which(splitrule.names == "indMaxMaxQCSpecific")
		splitrule <- splitrule.names[splitrule.idx]
	}
  if (grepl("indCat", fmly)) {
    splitrule.idx <- which(splitrule.names == "indCatSpecific")
    splitrule <- splitrule.names[splitrule.idx]
  }
    if (fmly == "class") {
      if (is.null(splitrule)) {
...
get.outcome.target <- function(family, yvar.names, outcome.target) {
  if (family == "regr" | family == "regr+" | family == "class" | family == "class+" | family == "mix+") {
    ...
    outcome.target <- match(outcome.target[1], yvar.names[1])
  }
    else {
...
get.yvar.type <- function(fmly, generic.types, yvar.names, coerce.factor = NULL) {
  if (fmly == "unsupv") {
    yvar.type <- NULL
	  ...
	  else if (grepl("HET", fmly) || grepl("indQuadC", fmly) || grepl("indQuad2C", fmly)  || grepl("indTSTC", fmly)  || grepl("indTST2C", fmly) || grepl("indNSRC", fmly) || grepl("indMOB0C", fmly) || grepl("CMB", fmly) || grepl("indMOBQC", fmly) || grepl("indMaxLC", fmly) || grepl("indMaxQC", fmly) || grepl("indMaxMaxQC", fmly) || grepl("indCat", fmly)) {
        yvar.type <- c("T", "G")
      }
	  else if (grepl("indLSC", fmly)) {
        yvar.type <- c("R", "G")
      }
		else {
...
parseFormula <- function(f, data, ytry = NULL, coerce.factor = NULL) {
  ...
  #Sami
  else if ((tolower(fmly) == "indsurvb")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized Survival formula incorrectly specified.")
    }
    if (length(yvar.names) == 3) {
      family <- "IndSurvB"
      ytry <- 3
    }
    else {
  ...
  else if ((tolower(fmly) == "indsurvcat")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized Survival formula incorrectly specified.")
    }
    if (length(yvar.names) == 3) {
  ...
  else if ((tolower(fmly) == "indlsc")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized least squares formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indLSC"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "het")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized Linear formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "HET"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indnsrc")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized NSR formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indNSRC"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indtstc")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized TST formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indTSTC"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indtst2c")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized TST2 formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indTST2C"
      ytry <- 2
    ...
    else if ((tolower(fmly) == "indmob0c")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized MOB0 formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indMOB0C"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "cmb")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized MOBL formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "CMB"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indmobqc")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized MOBQ formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indMOBQC"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indmaxlc")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized MAXL  formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indMaxLC"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indmaxqc")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized MAXQ formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indMaxQC"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indmaxmaxqc")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized MAXMAXQ formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indMaxMaxQC"
      ytry <- 2
      ...
      if (length(yvar.names) != 2){
        stop("Individualized MAXMAX Q formula incorrectly specified.")
      }
    }
  }
  else if ((tolower(fmly) == "indquadc")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized Quadratic formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indQuadC"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indquad2c")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized Quadratic 2 formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indQuad2C"
      ytry <- 2
  ...
  else if ((tolower(fmly) == "indcat")) {
    if (sum(is.element(yvar.names, names(data))) !=  length(yvar.names)) {
      stop("Individualized Categorical formula incorrectly specified.")
    }
    if (length(yvar.names) == 2) {
      family <- "indCat"
      ytry <- 2
