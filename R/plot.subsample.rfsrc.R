plot.subsample.irfsrc <- function(x, alpha = .01,
                          ...
                          ...)
{
  ...
  ##
  ##--------------------------------------------------------------
  if (sum(c(grepl("irfsrc", class(x))), grepl("subsample", class(x))) == 2) {
    subsample <- TRUE
  }
  else if (sum(c(grepl("irfsrc", class(x))), grepl("bootsample", class(x))) == 2) {
    subsample <- FALSE
  }
  else {
...
plot.subsample <- plot.subsample.irfsrc
